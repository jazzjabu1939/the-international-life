...
function render(){
  ...
  game.querySelectorAll('.bag-hotspot, .decoy-hotspot, .desk-hotspot, .console-hotspot, .note-hotspot, .earplug-hotspot, .interaction-hint').forEach(hotspot=>hotspot.remove());
  ...
  if(scene.id==='condo'){
    ...
    const earplugs=document.createElement('button'); earplugs.type='button'; earplugs.className='earplug-hotspot'; earplugs.setAttribute('aria-label','Take the earplugs from the bedside table'); earplugs.innerHTML=earplugsTaken?'<span>EARPLUGS</span><small>IN YOUR POCKET</small>':'<span>EARPLUGS</span><small>TAKE THEM</small>'; earplugs.disabled=earplugsTaken; earplugs.addEventListener('click',()=>{earplugsTaken=true;earplugs.disabled=true;earplugs.classList.add('collected');earplugs.innerHTML='<span>EARPLUGS</span><small>IN YOUR POCKET</small>';observation.hidden=false;observation.textContent='The bass still reaches the bedframe, but the earplugs may make sleep possible.';}); game.append(earplugs);
